...
# This creates a file named 'fake_account_model.pkl'
with open('fake_account_model.pkl', 'wb') as file:
    pickle.dump(model, file)

print("Model saved as fake_account_model.pkl. Download it from the folder icon on the left!")

import os
import pandas as pd
import streamlit as st

# This finds the folder where your script is located
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
FILE_PATH = os.path.join(BASE_DIR, "fake_account.csv")

@st.cache_data
def load_data():
    if os.path.exists(FILE_PATH):
        # Using sep='\t' because your CSV snippet showed tab separation
        return pd.read_csv(FILE_PATH, sep='\t', header=None, names=["user_id", "text"])
    else:
...
# 1. Load your data
df = pd.read_csv('fake_account.csv', sep='\t', header=None, names=['user_id', 'text'])
df = df.dropna() # Remove empty rows

# 2. Create labels (Since your file is 'fake_accounts', we assume these are all 1)
# In a real project, you'd combine this with the 'legitimate' data you showed earlier
df['label'] = 1
...
# 4. SAVE TO .PKL (This is what you need for GitHub)
with open('model.pkl', 'wb') as f:
    pickle.dump(model, f)
with open('vectorizer.pkl', 'wb') as f:
    pickle.dump(vectorizer, f)

print("Files 'model.pkl' and 'vectorizer.pkl' are ready. Download them from the file sidebar!")
...
st.title("ITBP Fake Account Detection Tool")

try:
    model, vectorizer = load_assets()
    st.success("Cybersecurity Model Loaded Successfully")

    user_input = st.text_area("Enter Social Media Post Content:")
    if st.button("Identify"):
        ...
        if prediction[0] == 1:
            ...
            # Add the Blockchain Hash here for the Problem Statement requirement
            import hashlib
            evidence = hashlib.sha256(user_input.encode()).hexdigest()
            st.write(f"Blockchain Evidence Hash: {evidence}")
        else:
            st.success("Account Verified as Legitimate")

except FileNotFoundError:
    st.warning("Please upload model.pkl and vectorizer.pkl to the GitHub repository.")
